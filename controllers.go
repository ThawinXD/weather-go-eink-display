...
// WeatherHandler returns an http.HandlerFunc that serves the /weather route.
func WeatherHandler(apiKey, latitude, longitude, password string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// compute hours for history request (min(currHour+7,24))
		hoursParam := currHour + 7
		if hoursParam > 24 {
			hoursParam = 24
		}

		resp2, err := client2.Get(fmt.Sprintf("https://weather.googleapis.com/v1/history/hours:lookup?key=%s&location.latitude=%s&location.longitude=%s&hours=%d", apiKey, latitude, longitude, hoursParam))
		if err != nil {
		...
		if err := json.Unmarshal(body2, &sourceWeather2); err != nil {
			...
			http.Error(w, "Failed to parse weather data for past 7 hours", http.StatusInternalServerError)
			return
		}
		...
		for _, slot := range fixedWindow {
			if data, exists := mergedHours[slot]; exists {
				...
				filledHours = append(filledHours, ForecastHour{
					...
					Temperature:          TemperatureValue{Celsius: -1},
					FeelsLikeTemperature: TemperatureValue{Celsius: -1},
					Humidity:             PercentValue{Percent: -1},
					RainChance:           PercentValue{Percent: -1},
				})
			}
		}

		weather := WeatherResponse{ForecastHours: filledHours}
...
// AqiHandler returns an http.HandlerFunc that serves the /aqi route.
func AqiHandler(apiKey, latitude, longitude, password string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Check bearer token
		authHeader := r.Header.Get("Authorization")
		if authHeader == "" || len(authHeader) < 7 || authHeader[:7] != "Bearer " {
